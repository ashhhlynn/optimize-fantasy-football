...
function getCaptain(data, sleeperdata) {
    let y = data.draftables.map((element) => {
        return {
            Name: element.displayName,
            Position: element.position,
...
async function fetchThursdayCaptain() {
    let sleeperdata = await getSleeperProjections()
    fetch('https://api.draftkings.com/draftgroups/v1/draftgroups/95260/draftables')
    .then((res)=> res.json())
    .then(data => {
        getCaptain(data, sleeperdata)
      
    })
}

async function fetchSundayClassic() {
    let sleeperdata = await getSleeperProjections()
    fetch('https://api.draftkings.com/draftgroups/v1/draftgroups/95253/draftables')
    .then((res)=> res.json())
    .then(data => {
        getClassic(data, sleeperdata)
